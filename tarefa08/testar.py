    ...
    def filter_text(self, text):
        ...
        for line in text.split("\n"):
            if ":" in line:
                ...
                line = f'{left}: {" ".join(sorted(right.split()))}'
            new_text += line + "\n"
        return new_text

    def compare_output(self, out, res):
        out = self.filter_text(out)
        res = self.filter_text(res)
        return self.compare_stripped(out, res)

    def teste_1_cidades_busca(self):
        cases = self.case_range("cidades{}.in", 1, 3)
        binary = self.make("cidades")
        self.test_cases(binary, cases)

    def teste_2_cidades_regiao(self):
        cases = self.case_range("cidades{}.in", 4, 6)
        binary = self.make("cidades")
        self.test_cases(binary, cases)

    def teste_3_cidades_remocao(self):
        cases1 = self.case_range("exemplo{}.in", 1, 1)
        cases2 = self.case_range("cidades{}.in", 7, 10)
        binary = self.make("cidades")
