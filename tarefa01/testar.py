...
class Task(BaseTask):
    def teste_1_capicua(self):
        cases = self.case_range("capicua{}.in", 1, 10)
        binary = self.compile(["capicua.c"])
        self.test_cases(binary, cases)

    def teste_2_parciais(self):
        cases = self.case_range("parciais{}.in", 1, 10)
        binary = self.compile(["parciais.c"])
        self.test_cases(binary, cases)

    def teste_3_sela(self):
        cases = self.case_range("sela{}.in", 1, 12)
