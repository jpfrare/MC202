...
class Literal:
    ...
    def satisfeito(self, solucao):
        return self.sinal == solucao[self.numero].sinal

    @staticmethod
    def criar_lista(linha):
        return [Literal(lit[0], int(lit[1:])) for lit in linha.split()]
...
class Clausula:
    ...
    def satisfeita(self, solucao):
        return any(literal.satisfeito(solucao) for literal in self.literais)


class Task(BaseTask):
    @staticmethod
    def ler_instancia(in_filename):
        with open(in_filename) as fp:
            _, n_var = map(int, fp.readline().split(maxsplit=1))
            clausulas = []
            for line in fp:
                if not line.strip():
                    continue
                peso, _, linha = line.split(maxsplit=2)
                clausulas.append(Clausula(int(peso), Literal.criar_lista(linha)))
        return clausulas, n_var

    @staticmethod
    def ler_solucao(out_filename, n_var):
        with open(out_filename) as fp:
            ...
            try:
                ...
                valor = int(linha)
                linha = fp.readline()
                solucao = Literal.criar_lista(linha)
            except:  # noqa
                raise AssertionError(
                    f"{out_filename}: formato de solução inválido: {linha[:20].strip()}"
                )
            assert [lit.numero for lit in solucao] == list(
                range(n_var)
            ), f"{out_filename}: formato inválido, saída deve listar todas alavancas em ordem"
            assert all(
                lit.sinal in ("+", "-") for lit in solucao
            ), f"{out_filename}: formato inválido, sinal das alavancas devem ser + ou -"
            return valor, solucao

    def compare_files(self, out, res):
        in_filename = out.replace(".out", ".in")
        clausulas, n_var = self.ler_instancia(in_filename)
        val_otimo, _ = self.ler_solucao(res, n_var)
        val_aluno, solucao_aluno = self.ler_solucao(out, n_var)
        val_real = sum(c.peso for c in clausulas if c.satisfeita(solucao_aluno))
        assert (
            val_real == val_aluno
        ), f"{out}: o valor reportado é {val_aluno}, mas a solução encontrada vale {val_real}"
        assert (
            val_real == val_otimo
        ), f"{out}: encontrou solução com valor {val_real}, mas a maior improbabilidade é {val_otimo}"
        return True

    def teste_1_improbabilidade_minimo(self):
        cases = self.case_range("improbabilidade{}.in", 1, 5)
        binary = self.make("improbabilidade")
        self.test_cases(binary, cases)

    def teste_2_improbabilidade_pequeno(self):
        cases = self.case_range("improbabilidade{}.in", 6, 7)
        binary = self.make("improbabilidade")
        self.test_cases(binary, cases)

    def teste_3_improbabilidade_medio(self):
        cases = self.case_range("improbabilidade{}.in", 8, 9)
        binary = self.make("improbabilidade")
        self.test_cases(binary, cases)

    def teste_4_improbabilidade_grande(self):
        cases = self.case_range("improbabilidade{}.in", 10, 10)
        binary = self.make("improbabilidade")
