...
class Task(BaseTask):
    ...
    def teste_1_centroide(self):
        cases1 = self.case_range("centroide{}.in", 1, 1)
        cases2 = self.case_range("centroide{}.in", 2, 7)
        binary = self.make("centroide")
        self.ABS_TOLERANCE = 0.00101
        self.test_cases(binary, cases1)
        self.ABS_TOLERANCE = 0.001
        self.test_cases(binary, cases2)

    def teste_2_quase_palindromo(self):
        cases = self.case_range("quase_palindromo{}.in", 1, 5)
        binary = self.make("quase_palindromo")
        self.test_cases(binary, cases)

    def teste_3_recompensa(self):
        tests = [
            ("2 1", "0.167"),
            ("2 2", "0.194"),
            ("3 9", "0.227"),
        ...
        self.number_cases += len(tests)
        binary = self.make("recompensa")
        for inputvalue, outputvalue in tests:
            self.input_output(binary, inputvalue, outputvalue)
            self.passed_cases.add(inputvalue)
