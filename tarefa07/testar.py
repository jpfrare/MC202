...
class Task(BaseTask):
    ...
    def teste_0_configurar_duplas_txt(self):
        ...
        if self.current_ra is None:
            m = re.search(r"/ra(\d{6})/", os.path.abspath("."))
            self.current_ra = m.group(1) if m else None
        if self.current_ra is not None:
            assert (
                self.current_ra in elaborada
            ), "seu número de RA deve estar na lista de em quem elaborou"
        if self.current_ra != submit_ra:
            print("\n####################  ATENÇÃO  ####################\n")
        ...
        self.submit_ra = submit_ra  # marca como configurada

    def teste_1_and_or(self):
        cases = self.case_range("and_or{}.in", 1, 1)
        binary = self.make("otimizar")
        self.test_cases(binary, cases)

    def teste_2_not(self):
        cases = self.case_range("not{}.in", 1, 2)
        binary = self.make("otimizar")
        self.test_cases(binary, cases)

    def teste_3_equivalente(self):
        cases = self.case_range("equivalente{}.in", 1, 1)
        binary = self.make("otimizar")
        self.test_cases(binary, cases)

    def teste_4_demorgan(self):
        cases = self.case_range("demorgan{}.in", 1, 2)
        binary = self.make("otimizar")
        self.test_cases(binary, cases)
    ...
    def teste_6_grande(self):
        cases = self.case_range("grande{}.in", 1, 10)
        binary = self.make("otimizar")
        self.test_cases(binary, cases)
